...
- The project now contains both a historical main suite and a clean current-workspace rerun, each with paired metrics, LaTeX tables, figures, and markdown reports suitable for audit and paper drafting.
...
- `fedgrid_topo_proto` is the only currently promising variant, but its sign on the main benchmark is not yet stable enough for a definitive superiority claim.
