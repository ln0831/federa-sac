"""Evaluate topology-shift impact for baseline vs GNN checkpoints.

Patched to support:
- baseline FMASAC checkpoints (actors only)
- original local bus-GNN checkpoints (bus_encoders as list)
- Scheme-B global shared bus-GNN checkpoints (bus_encoders as a single state_dict)

It reconstructs actor input dims from the checkpoint instead of assuming raw env
observation dims, then rebuilds the optional bus encoder and appends its
embeddings at evaluation time.
"""
...
def load_policy(env, ckpt_path: str, device):
    """Load actors plus optional bus encoder from a checkpoint.

    Returns:
        actors, bus_encoder, expected_state_dims
    """
        ...
        if embed_dim > 0:
            ...
            if isinstance(bus_blob, dict):
                ...
                bus_encoder = build_global_bus_encoder(
                    ...
                    # Current training package saves Scheme-B with base topology enabled.
...
def _build_actor_inputs(obs_list, bus_encoder, expected_state_dims, device):
    ...
    for i, o in enumerate(obs_tensors):
        ...
        # Fallback padding if the checkpoint expects extra dims but no encoder was found.
...
def eval_once(env, actors, bus_encoder, expected_state_dims, device, episodes: int, steps: int) -> Tuple[List[Dict], Dict[str, float]]:
    rows: List[Dict] = []
...
def main():
    ...
    ap.add_argument("--baseline_name", type=str, default="baseline", help="Label used in output CSV for the baseline model")
    ap.add_argument("--gnn_name", type=str, default="gnn", help="Label used in output CSV for the compared model (e.g., gnn_nobus)")
    ...
    for topo_mode in ["static", "random_reset"]:
        # Use two fresh envs so baseline and GNN see the same distribution/seed setup.
        env_b = load_env(
            case=args.case,
            num_agents=num_agents,
        ...
        rows_b, sum_b = eval_once(env_b, actors_b, enc_b, dims_b, device, args.episodes, steps)

        actors_g, enc_g, dims_g = load_policy(env_g, args.gnn_ckpt, device)
        rows_g, sum_g = eval_once(env_g, actors_g, enc_g, dims_g, device, args.episodes, steps)
