...
class Opts:
    def __init__(self):
        parser = argparse.ArgumentParser(description="FedGrid-v4 GNN-FMASAC: Clustered-Distilled Federated Topology Control")
        ...
        add_bool_arg(parser, '--fed_clustered', default=False,
                     help='Enable community-aware clustered federation before parameter mixing.')
        parser.add_argument('--fed_cluster_knn', type=int, default=2, help='Top-k neighbors used to derive client communities from federated affinity.')
        parser.add_argument('--fed_cluster_threshold', type=float, default=0.58, help='Minimum affinity needed to connect two clients into the same cluster.')
        parser.add_argument('--fed_max_clusters', type=int, default=4, help='Upper bound on the number of dynamic client clusters.')
        parser.add_argument('--fed_inter_cluster_scale', type=float, default=0.08, help='Residual cross-cluster aggregation weight retained after cluster masking.')
        parser.add_argument('--fed_cluster_self_boost', type=float, default=0.10, help='Additional self-weight injected after cluster masking to stabilize personalization.')
        parser.add_argument('--fed_distill_coef', type=float, default=0.0, help='Peer-distillation coefficient applied after each federated round. 0 disables distillation.')
        parser.add_argument('--fed_distill_steps', type=int, default=1, help='How many extra distillation optimization steps to run after a federated round.')
        parser.add_argument('--fed_distill_batch_size', type=int, default=128, help='Replay-anchor batch size used for post-aggregation peer distillation.')
        ...
        self.fed_clustered = bool(args.fed_clustered)
        self.fed_cluster_knn = int(args.fed_cluster_knn)
        self.fed_cluster_threshold = float(args.fed_cluster_threshold)
        self.fed_max_clusters = int(args.fed_max_clusters)
        self.fed_inter_cluster_scale = float(args.fed_inter_cluster_scale)
        self.fed_cluster_self_boost = float(args.fed_cluster_self_boost)
        self.fed_distill_coef = float(args.fed_distill_coef)
        self.fed_distill_steps = int(args.fed_distill_steps)
...
def main():
    ...
    fed_last_clusters = list(range(opts.num_agents))
    fed_last_distill_loss = 0.0
    fed_last_payload_mib = 0.0
    ...
    for epoch in range(opts.epochs):
        ...
        if fed_last_stats:
            ...
            tb_logger.add_scalar('fed/cluster_count', float(fed_last_stats.get('cluster_count', 1.0)), epoch)
            tb_logger.add_scalar('fed/distill_loss', float(fed_last_stats.get('distill_loss', 0.0)), epoch)
            tb_logger.add_scalar('fed/payload_mib', float(fed_last_stats.get('payload_mib', 0.0)), epoch)
        ...
        if should_run_federated_round(
            ...
            with torch.no_grad():
                ...
                if bool(getattr(opts, 'fed_clustered', False)):
                    cluster_ids = derive_client_clusters(
                        0.5 * (W + W.transpose(0, 1)),
                        knn=int(getattr(opts, 'fed_cluster_knn', 2)),
                        threshold=float(getattr(opts, 'fed_cluster_threshold', 0.58)),
                        max_clusters=int(getattr(opts, 'fed_max_clusters', 4)),
                        active_mask=active_mask,
                    )
                    W = mask_weights_by_clusters(
                        W,
                        cluster_ids,
                        inter_cluster_scale=float(getattr(opts, 'fed_inter_cluster_scale', 0.08)),
                        self_boost=float(getattr(opts, 'fed_cluster_self_boost', 0.10)),
                    )
                    W = apply_participation_mask(
                        W,
                        active_mask,
                    ...
                    cluster_ids = [0 for _ in range(opts.num_agents)]
                fed_last_clusters = list(cluster_ids)

                active_actor_bits = estimate_module_payload_bits(
                    [actors[i] for i in active_ids],
                    exclude_prefixes=('l1.', 'mean_layer.', 'log_std_layer.'),
                )
                active_critic_bits = estimate_module_payload_bits(
                    [critics[i] for i in active_ids],
                    exclude_prefixes=('l1.', 'c_head.', 'e_head.'),
                )
                if bus_encoders is not None and (not isinstance(bus_encoders, GlobalBusGCNEncoder)):
                    active_enc_bits = estimate_module_payload_bits(
                        [bus_encoders[i] for i in active_ids],
                        exclude_keys=('adj_norm',),
                    )
                else:
                    active_enc_bits = 0
                roundtrip_bits = 2 * int(active_actor_bits + active_critic_bits + active_enc_bits)
                fed_last_payload_mib = float(roundtrip_bits / 8.0 / (2 ** 20))
                ...
                fed_last_stats['cluster_count'] = float(len(set(int(c) for c in cluster_ids)))
                fed_last_stats['cluster_ids'] = cluster_ids
                fed_last_stats['payload_mib'] = float(fed_last_payload_mib)
            ...
            fed_last_distill_loss = 0.0
            if float(getattr(opts, 'fed_distill_coef', 0.0)) > 0.0 and buffer.size >= max(8, int(getattr(opts, 'fed_distill_batch_size', 128))):
                if opts.use_context:
                    distill_sample = buffer.sample(int(getattr(opts, 'fed_distill_batch_size', 128)))
                    d_obs = [x.to(opts.device) for x in distill_sample[0]]
                else:
                    distill_sample = buffer.sample(int(getattr(opts, 'fed_distill_batch_size', 128)))
                    d_obs = [x.to(opts.device) for x in distill_sample[0]]
                if opts.use_bus_gnn and bus_encoders is not None:
                    with torch.no_grad():
                        d_z = compute_bus_embeddings(bus_encoders, d_obs, detach=True)
                    d_obs_aug = [torch.cat([o, z], dim=1) for o, z in zip(d_obs, d_z)]
                else:
                    d_obs_aug = d_obs
                fed_last_distill_loss = distill_actors_from_peers(
                    actors,
                    actor_optims,
                    d_obs_aug,
                    W.to(opts.device),
                    coef=float(getattr(opts, 'fed_distill_coef', 0.0)),
                    steps=int(getattr(opts, 'fed_distill_steps', 1)),
                    log_std_weight=float(getattr(opts, 'fed_distill_logstd_weight', 0.25)),
                    grad_clip=float(getattr(opts, 'grad_clip', 0.0)),
                ...
                fed_last_stats['distill_loss'] = 0.0
            ...
            if val_ret > best_ret:
                ...
                save_dict = {
                    ...
                    'fedgrid_cfg': {
                        ...
                        'max_clusters': int(getattr(opts, 'fed_max_clusters', 4)),
                        'inter_cluster_scale': float(getattr(opts, 'fed_inter_cluster_scale', 0.08)),
                        'cluster_self_boost': float(getattr(opts, 'fed_cluster_self_boost', 0.10)),
                        'distill_coef': float(getattr(opts, 'fed_distill_coef', 0.0)),
