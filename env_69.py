...
# 导入 loader
from case69_loader import create_case69
...
class DistNetEnv(gym.Env):
    def __init__(
        ...
        **kwargs,
        ...
        # 1. 构建网络
        print("[Env] 正在通过 Python Loader 构建 IEEE 69 网络...")
        self.net_orig = create_case69()
    ...
    def _init_modifications(self):
        ...
        # 添加 PV: 每隔 6 个节点
        cnt_pv = 0
        for bus in valid_buses[::6]: 
            pp.create_sgen(self.net_orig, bus, p_mw=0.0, q_mvar=0, sn_mva=1.0, name=f"PV_{bus}", type="PV")
            cnt_pv += 1
        
        # 添加 SVC: 每隔 10 个节点
        cnt_svc = 0
        for bus in valid_buses[::10]:
            pp.create_shunt(self.net_orig, bus, q_mvar=0, p_mw=0, name=f"SVC_{bus}")
            cnt_svc += 1
                
        print(f"[Env] IEEE 69: 已添加 {cnt_pv} 个 PV (1.0 MVA) 和 {cnt_svc} 个 SVC。")
