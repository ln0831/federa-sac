...
class Opts:
    def __init__(self):
        parser = argparse.ArgumentParser(description="GNN-FMASAC: Topology-Aware Voltage Control (Fair Compare)")
        ...
        parser.add_argument('--fed_mode', type=str, default='none', choices=['topo','fedavg','none'],
                            help='Federated mixing of shared parameters across agents.')
...
def main():
    ...
    for epoch in range(opts.epochs):
        ...
        # =============== Structure-2: topology-weighted federated aggregation ===============
        if opts.fed_mode != 'none' and opts.fed_round_every > 0 and ((epoch + 1) % opts.fed_round_every == 0):
            ...
            if bool(getattr(opts, 'fed_use_base_topology', True)) and hasattr(be, 'net_orig'):
                ...
                net_for_w = be.net
            with torch.no_grad():
                W = get_agent_adjacency(net_for_w, be.areas, device='cpu', mode=opts.adj_mode)
                if opts.fed_mode == 'fedavg':
                    W = torch.ones_like(W)

            # bus encoder:
            # - global shared encoder (Scheme-B): already shared, no per-agent mixing needed
            # - local encoders (legacy): mix parameters across agents, keep adjacency buffers intact
            ...
            # actor/critic: only mix shared hidden layers (exclude input layer and heads)
            topology_weighted_mix(
            ...
            # sync targets after aggregation
            for tc, c in zip(target_critics, critics):
                hard_update(tc, c)
            if target_bus_encoders is not None and bus_encoders is not None:
                if isinstance(bus_encoders, GlobalBusGCNEncoder):
            ...
            # clear optimizer momentum to avoid stale states after parameter jumps
