...
class DistNetEnv(gym.Env):
    def __init__(
        ...
        **kwargs,
        ...
        # 1. 加载 IEEE 33 节点标准模型
        print("[Env] 正在加载 IEEE 33 节点系统 (pandapower内置)...")
        self.net_orig = pn.case33bw()
    ...
    def _init_modifications(self):
        ...
        # 添加 PV: 每隔 3 个节点放一个 (约 10 个 PV)
        # 容量设为 0.8 MVA，保证有能力调节电压
        cnt_pv = 0
        for bus in valid_buses[::3]: 
            pp.create_sgen(self.net_orig, bus, p_mw=0.0, q_mvar=0, sn_mva=0.8, name=f"PV_{bus}", type="PV")
            cnt_pv += 1
        
        # 添加 SVC: 每隔 5 个节点放一个 (约 6 个 SVC)
        cnt_svc = 0
        for bus in valid_buses[::5]:
            pp.create_shunt(self.net_orig, bus, q_mvar=0, p_mw=0, name=f"SVC_{bus}")
            cnt_svc += 1
                
        print(f"[Env] IEEE 33: 已添加 {cnt_pv} 个 PV 和 {cnt_svc} 个 SVC。")
    ...
    def step(self, actions):
        for i, area_buses in enumerate(self.areas):
            ...
            for idx in area_pv:
                ...
                # Action [-1, 1] -> P [0, s_max]
                # 注意：PV P通常不可调（取决于光照），这里假设是全控或者有储能配合
                # 为了简化，假设 P 可削减 (Curresailment) 或者可调度
            ...
            for idx in area_svc:
                ...
                # 范围 [-1, 1] -> [-1.0 MVar, 1.0 MVar]
                self.net.shunt.at[idx, 'q_mvar'] = act[ptr] * 1.0
                ptr += 1
        ...
        if self.success:
            ...
            for i, area_buses in enumerate(self.areas):
                ...
                # 系数设计逻辑：
                # Loss ~ 0.1 MW -> * 10 = 1.0
                # Linear Viol ~ 0.01 p.u. -> * 200 = 2.0 (权重高于 Loss)
                # Squared Viol (严重) -> * 1000 (极高权重，强制安全)
                raw_reward = - (p_loss_val * 10.0) - (200.0 * linear_viol) - (1000.0 * squared_viol)
                
                # 缩放到 -1 ~ 0 之间，方便 Critic 拟合
    ...
    def _get_obs(self):
        ...
        for area_buses in self.areas:
            ...
            # 归一化策略:
            # P/Q: 通常在 0~5 MW 之间，除以 5 归一化到 0~1
            p_norm = p / 5.0
            q_norm = q / 5.0
            
            # V: 在 0.9~1.1 之间。
            # (V - 1.0) * 20 -> 映射 0.95 到 -1.0，1.05 到 1.0
            # 这样放大电压的微小变化，让神经网络更容易感知
            v_norm = (v - 1.0) * 20.0
